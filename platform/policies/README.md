...
|---|---|---|
| `wdac/` | `zd-base.xml` (allow Microsoft + ZD code-signing chain + catalogues; deny all), `catalogs/<tool>-<version>.cat` signed per third-party release | DSC (`CiTool`) |
...
| `wazuh-rules/` | ZD rule packs: `zd-openbao`, `zd-wdac`, `zd-sf`, `zd-s3`, `zd-ad-tier` | Wazuh manager on `siem-01` |
| `sysmon.xml` | Olaf Hartong `sysmon-modular` baseline with ZD tuning | GPO |
