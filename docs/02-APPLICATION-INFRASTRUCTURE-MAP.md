...
|---|---|---|---|---|---|
| `hv-01` … `hv-03` (→ `hv-05`) | Hyper-V + S2D cluster `zd-hvc-a` | 1U EPYC 32c, 256 GB, 2 × 3.84 TB NVMe (cache/OS), 4 × 20 TB HDD (capacity), 2 × 25 GbE RDMA, TPM 2.0, IPMI | Windows Server 2025 Datacenter, Server Core | MGMT 10, STORAGE 40 | Secure Boot, BitLocker TPM+PIN, WDAC, Credential Guard, HGS-attested |
...
| Host | Role | Notes |
|---|---|---|
| `hv-b01`, `hv-b02` | Hyper-V cluster `zd-hvc-b`; Hyper-V Replica target | same spec minus GPU; 6 × 20 TB each |
...
| `sql-01` | A | 16 / 128 GB / OS 100 GB + data 2 TB NVMe-tier CSV + log 500 GB | SQL Server 2022 Standard, AG `zd-ag1` primary: `umairv3_db`, `FarmerFacilitatorDb`, `ZdConsole` | `gmsa-sql$` | DATA 31 | Native backups every 15 min log / 6 h diff / 24 h full → `zd-backups`; Hyper-V Replica |
| `sql-02` | B | 16 / 128 GB / same | AG async secondary; readable for reporting | `gmsa-sql$` | DATA-B | Local copy of backups |
| `pg-01` | A | 8 / 64 GB / OS 100 GB + data 1 TB | PostgreSQL 17 + PostGIS 3.5: `pgstac`, `zd_geo`, `zd_ml`, `zd_console_events` | `gmsa-pg$` | DATA 31 | `pg_basebackup` nightly + WAL via wal-g → `zd-backups`; streaming replica on `pg-02` (B) in Phase 5 |
| `sw-master-01..03` | A | 4 / 16 GB / 100 GB + volume disks on S2D capacity tier (3 × 60 TB) | SeaweedFS master + volume + filer + S3 gateway, replication `001` | `gmsa-seaweed$` | DATA 31 | Volumes are the storage; `zd-backups` replicated to B |
| `sw-b01` | B | 4 / 16 GB / 100 TB | SeaweedFS replica; object lock 90 d on `zd-backups` | `gmsa-seaweed$` | DATA-B | — |
...
| `siem-01` | A | 8 / 32 GB / 1 TB | Ubuntu; Wazuh manager + indexer + dashboard; Loki long-term index | `svc-wazuh` (AD, SSSD) | SEC 12 | Kopia of indexes → `zd-backups`; Hyper-V Replica |
...
| `forgejo-01` | A | 4 / 8 GB / 500 GB | Forgejo mirror of all GitHub repos + Actions runner | `gmsa-forgejo$` | BUILD 32 | Kopia → `zd-backups` |
...
| `Loki` | 1 | 3100 | `gmsa-loki$` | `zd-logs` bucket for chunks | `/ready` | ingest errors |
...
| `ZdConsoleApi` | 3 | 5200 | `gmsa-console$` | SQL `ZdConsole`, OpenBao, Git (Forgejo), NATS, SF client API | `/api/v1/health` | 5xx; reconcile lag |
| `ZdConsoleWeb` | 2 | 3200 | `gmsa-console$` | `ZdConsoleApi` | `/` | — |
| `ZdReconciler` | 1 stateful (3 replicas) | 5210 | `gmsa-reconciler$` | Git, SF client API, WMI to `hv-*`, SeaweedFS admin, OpenBao, DSC | SF replica health | desired ≠ actual > 15 min |
| `Titiler` | 2 | 8100 | `gmsa-geo$` | `zd-sentinel`, `pg-01` (pgstac) | `/healthz` | — |
| `Martin` | 2 | 3300 | `gmsa-geo$` | `pg-01` (`zd_geo`) | `/health` | — |
...
| Shapefile ingest, exports, AI briefings | Azure Functions host, NATS trigger `zd.ingest.mills.*` | 2 | — | `gmsa-mills-jobs$` | `sql-01`, `zd-survey-pictures`, `zd-artifacts` | as above | queue age < 5 min |
| Survey images (`/map-qc/image`, 1.16 M files) | SeaweedFS `zd-survey-pictures` | — | S3 | `gmsa-mills$` (read), `gmsa-mills-jobs$` (write) | — | `s3/creds/mills-pictures` (30 d rotating) | — |
| Corporate survey pictures | SeaweedFS `zd-survey-pictures/corporate/` | — | S3 | as above | | | |
...
| Backups | SQL native → `zd-backups/sql/umairv3_db/` (lock 35 d) → Site B (lock 90 d); Kopia of `zd-survey-pictures` → `zd-backups/kopia/pictures/` | | | `gmsa-backup$` (PUT only) | | | drill monthly |
...
| Mobile app (Expo) | phones | talks to `loans.zaraatdost.pk` via Caddy; certificate pinning to the ZD chain; repointed from AWS in Phase 4 |
| Applicant images | `zd-survey-pictures/loans/` | same bucket, own prefix, own S3 credential |

### D.3 AGIS (`Adilmunawar/AGIS`, Next.js + Firebase)

| Component | Runs as | Notes |
|---|---|---|
| AGIS web | SF guest exe ×2, `gmsa-agis$` | Firebase Auth → AD FS OIDC and Firestore → PostgreSQL `zd_geo` are Phase 4 migrations; until then Firebase stays external behind the egress proxy with keys in `kv/agis/firebase` |
| Map layers | Martin (vector tiles from `zd_geo`), TiTiler (rasters from `zd-sentinel`) | replaces per-page GeoJSON |
...
|---|---|---|---|---|---|
| Sentinel mirror (S1 GRD, S2 L2A, Punjab/Sindh AOIs, daily) | `gpu-01` (CPU) | Dagster schedule → `eodag` → COG → `zd-sentinel`; pgstac insert; NATS `zd.sentinel.scene.landed` | Copernicus via egress proxy | `zd-sentinel/{s1,s2}/{tile}/{date}.tif`; STAC items | `svc-ml` (AD, SSSD); `s3/creds/ml-sentinel` |
| v5 land-use classifier (XGBoost; P00–P23 optical, M00–M23 radar, phenology) | `gpu-01` | Dagster assets: `parcels` → `s2_periods` → `s1_periods` → `phenology` → `v5_train_table` → `v5_model` → `parcel_predictions`; MLflow run per version (`v5.3` next) | `zd_geo.parcels`, `zd-sentinel`, GT tables in `zd_ml` | `zd_ml.v5_features`, `zd_ml.v5_predictions` (per season), model in MLflow → `zd-ml/models/v5/` | `svc-ml` |
| SegFormer-B5 land use (8 classes) | `gpu-01` (GPU) | Ray Train job from Dagster; checkpoints to `zd-ml/checkpoints/segformer/` | `zd-sentinel`, patches cache on local NVMe | prediction rasters → `zd-rasters/landuse/{aoi}/{season}.tif` | `svc-ml` |
| Sugarcane segmentation (UNet-CBAM, HRNet-W48) | `gpu-01` (GPU) | Ray Train; `final_finetune.py` becomes a Dagster op | as above | `zd-rasters/sugarcane/` | `svc-ml` |
| Boundary vectorisation (`boundary_to_polygons_ram_safe.py`) | `gpu-01` (CPU, Ray tasks per tile) | Dagster asset `parcel_polygons` | HRNet boundary rasters | `zd_geo.parcels_candidate` (PostGIS), QA in AGIS | `svc-ml` |
| Harvest detection (per-parcel NDVI drop) | `gpu-01` | Dagster sensor on `zd.sentinel.scene.landed` | `zd-sentinel`, `zd_geo.parcels` | `zd_ml.harvest_events` → Mills dashboard harvest pace | `svc-ml` |
| Inference endpoint (parcel classifier) | `gpu-01`, Ray Serve | behind Caddy at `ml.zaraatdost.pk/v5/predict`, AD FS-protected | | | `svc-ml` |
| Notebooks | `gpu-01`, JupyterHub | AD FS login; per-user GPU memory limits via Ray; `zd-ml` mounted | | | per user |
| Backups | MLflow DB (`zd_ml`) via wal-g; `zd-ml` and `zd-rasters` via Kopia (weekly, since they are reproducible); `zd-sentinel` **not** backed up (re-pullable) — documented exception | | | | `gmsa-backup$` |

### D.5 Zaraat Dost AI (inside Mills)

| Component | Notes |
|---|---|
| Anthropic API calls | from `MillsApi` only, via egress proxy, key `kv/mills/anthropic`, every prompt/answer to Loki (`zd.ai.audit`), 90-day retention |
...
See `05-CONTROL-PLANE.md`. Database `ZdConsole` on `sql-01`; events in `zd_console_events` on `pg-01`; Git state in Forgejo (mirrored to GitHub `ZaraatDost/zd-cloud-gitops`, private).
...
| `ZdConsole` | SQL Server | same | small | | | |
| `pgstac`, `zd_geo`, `zd_ml`, `zd_console_events` | PostgreSQL | `pg-01` | 200 GB → 2 TB | `gmsa-pg$` | WAL continuous | |
| `zd-survey-pictures` | SeaweedFS | `sw-*` | ~1.2 TB → 3 TB | `gmsa-seaweed$` | Kopia weekly + daily incremental | — |
| `zd-rasters` | SeaweedFS | | 2 TB → 10 TB | | Kopia monthly | |
| `zd-sentinel` | SeaweedFS | | 5 TB → 30 TB | | none (re-pullable) | |
| `zd-ml` | SeaweedFS | | 500 GB → 5 TB | | Kopia weekly | |
| `zd-artifacts` | SeaweedFS | | small | | replicated | WORM 365 d |
| `zd-backups` | SeaweedFS | A + B | 3.2 TB → 8 TB | `gmsa-backup$` (PUT) | is the backup | WORM 35 d (A), 90 d (B) |
| `zd-logs` | SeaweedFS | | 1 TB → 4 TB | `gmsa-loki$` | replicated to B | WORM 400 d |
