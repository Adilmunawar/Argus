...
`platform/console/prototype/tests/run-tests.js`, run in headless Chromium 141 against the real rendered DOM. Nine suites, 51 assertions. Exit code is the failure count, so CI can gate on it.
...
v1 failed 11 of 51. Every one is listed, including the four that turned out to be defects in my own test harness — those are the most instructive, because a test that lies is worse than no test.
