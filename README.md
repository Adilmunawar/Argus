...
[![console tests](https://img.shields.io/badge/console%20tests-153%2F153-046c4e)](platform/console/prototype/tests/run-tests.js)
...
[![status](https://img.shields.io/badge/status-planning%20%C2%B7%20v0.6.0-b07f23)](CHANGELOG.md)
...
| **Control** | Web console (.NET 10 + Next.js) · `argus` PowerShell module and CLI · a GitOps repository that is the only writer of production |
| **Targets** | RPO 15 min · RTO 8 h for full site loss · 99.9% · under 10 min from `git push` to signed production deploy |
| **Exceptions** | Three Linux hosts, each justified by an ADR: the GPU node, the Wazuh SIEM manager, the Guacamole gateway. OPNsense appliances are not counted, being appliances rather than servers. |
| **Owner** | Adil Munawar, ML Research & Development |
| **Status** | Planning · v0.6.0 · 8 September 2026 |
...
12. [`docs/11-CONSOLE-UX-BENCHMARK-AND-BUGS.md`](docs/11-CONSOLE-UX-BENCHMARK-AND-BUGS.md): AWS/GCP/Azure benchmark, the test suite, every bug found and the fix plan. `node platform/console/prototype/tests/run-tests.js`: currently 153/153.
...
platform/policies/  WDAC, GPO baselines, firewall/IPsec and Wazuh rules; enforced and versioned
...
node platform/console/prototype/tests/run-tests.js  # 153 assertions in headless Chromium
```
